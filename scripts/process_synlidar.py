...
# USGS_2_SYNLIDAR = { i:0 for i in range(256)}
# USGS_2_SYNLIDAR.update({2:2, 9:9, 17:17})

def _get_parser():
    import argparse
    parser = argparse.ArgumentParser(description='Tiling Point Cloud')
    parser.add_argument('--block_width', type=float, default=128, help='Block Width (meters)')
...
def _main():
    ...
    if in_path.suffix in (".laz",".las"):
        ...
        coord = np.stack([las.X, -las.Z, las.Y], axis=1).astype(np.float64)* scales + offsets
        xyz_min = np.min(coord, axis=0)
        xyz_max = np.max(coord, axis=0)
        xyz_mean = 0.5*(xyz_min + xyz_max)
        
        # feat = np.stack([las.intensity, las.return_num], axis=1)
        feat = np.zeros((coord.shape[0], 2), dtype=np.float32)
        label = np.array(las.classification, dtype=np.uint8)
        
        # label = np.vectorize(USGS_2_SYNLIDAR.get)(label)
